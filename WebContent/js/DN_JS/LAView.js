...
sap.ui.model.json.JSONModel.extend("LAView", {
	...
	createLesseeApprovalForm: function (oController) {
		...
		var labHardCode = new sap.ui.commons.Label({
			text: "Customer ID:",
			wrapping: true,
			required: true,
		...
		var labLESSEERef1 = new sap.ui.commons.Label({
			text: "DN / JS Reference:",
			wrapping: true
		});
		var labLESSEERef2 = new sap.ui.commons.Label({
			text: "1 per line, 25 max",
			wrapping: true
		}).addStyleClass("font10");
		var labLESSEERef3 = new sap.ui.commons.Label({
			text: "Use Enter to navigate to the next line",
			wrapping: true
		}).addStyleClass("font10");
		...
		var refFlex = new sap.m.FlexBox({
			items: [labLESSEERef1, labLESSEERef2, labLESSEERef3],
			direction: "Column",
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M3 S4",
				linebreak: true,
		...
		var labCity = new sap.ui.commons.Label({
			text: "City:",
			wrapping: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M3 S4",
		...
		var labPeriod = new sap.ui.commons.Label({
			text: "Off-Hire Period:",
			wrapping: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M3 S4",
				linebreak: true,
		...
		var labUnitNo1 = new sap.ui.commons.Label({
			text: "Unit Number:",
			wrapping: true
		});
		var WarningText =
			"To view the status: For individual containers, please enter the 4 letter prefix and 7 digit number for each container. (Eg. SEGU3000511). For multiple containers (Max 25), press the Enter key after each one, or copy and paste the list to the entry field";

		var labUnitNo2 = new sap.ui.commons.Label({
			text: WarningText,
			wrapping: true
		}).addStyleClass("font10");
		...
		/*var labReleaseRef4 = new sap.ui.commons.Label({text: "Use Shift + TAB to navigate to the previous line)",
			wrapping: true}).addStyleClass("font10");*/
		var unitFlex = new sap.m.FlexBox({
			items: [labUnitNo1, labUnitNo2 /*,labUnitNo3*/ ],
			direction: "Column",
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M3 S4",
				linebreak: true,
		...
		var labStatus = new sap.ui.commons.Label({
			text: "Status:",
			wrapping: true,
			visible: false,
			layoutData: new sap.ui.layout.GridData({
		...
		var labUnitType = new sap.ui.commons.Label({
			text: "Unit Type:",
			wrapping: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M3 S4",
		...
		rgStatus.addItem(oItem2);
		oItem2 = new sap.ui.core.Item({
			text: "Available Billing",
			key: "AVAILABLE BILLING"
		});
		rgStatus.addItem(oItem2);
		oItem2 = new sap.ui.core.Item({
			text: "Supersede by Joint Survey",
			key: "SUPERSEDED BY JOINT SURVEY"
		});
		rgStatus.addItem(oItem2);
	...
	setLesseApForm: function () {
		...
		var btnExpPdf = new sap.m.Button({
			...
			press: function () {
				...
				var arrColmnName = ['DN/JS', 'Date', 'Unit Number', 'Unit Type', 'Description', 'Off-Hire Location', 'Off-Hire Date', "Lease",
					"Amount", "Tax", "Curency"
				]; //COLUMN NAME
		...
		oLesseeTable.addColumn(new sap.ui.table.Column({
			...
			}).addStyleClass("wraptextcol"), //tblHeaderCustomTV
	...
	gotoApproveEstimate: function () {
		...
		if (DNJS_ApproveData.length > 0) {
			...
			var aler =
				"No DN/JS selected for approval. Please select atleast 1 DN/JS for approval \n(by checking the checkbox against the required DN/JS) and retry.";
			...
			if (resultdata != undefined) {
				if (resultdata.results.length > 0) {
					for (var j = 0; j < resultdata.results.length; j++) {
						oidAutoCmpltlesseApHardCode.addItem(new sap.ui.core.ListItem({
	...
	getOnlineCutomerListError: function (e) {
		busyDialog.close();
		var oidAutoCmpltlesseApHardCode = sap.ui.getCore().byId("idAutoCmpltlesseApHardCode");
		oidAutoCmpltlesseApHardCode.insertItem((new sap.ui.core.ListItem({
