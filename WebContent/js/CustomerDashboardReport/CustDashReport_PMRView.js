...
sap.ui.model.json.JSONModel.extend("CustReportPMRView", {
	...
	createCustReportPMRView: function () {
		...
		var oLabelLocation = new sap.ui.commons.Label({
			text: "Location:",
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
				linebreak: true,
		...
		var oLabelUnitType = new sap.ui.commons.Label({
			text: "Unit Type: ",
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
				linebreak: true,
		...
		var oLabelDate = new sap.ui.commons.Label({
			text: "Period:",
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
				linebreak: true,
			...
			function (data, response) {
				...
				for (var i = 0; i < dataLen; i++) {
					if (data.results[i].CustName != "") {
						...
						oComboCustomerCustPMR.addItem(new sap.ui.core.ListItem({
							text: data.results[i].CustName,
							key: data.results[i].Partner,
							additionalText: data.results[i].Partner
						}));
					}
				}
				...
				OData.request({
					...
					function (data, response) {
						...
						for (var j = 0; j < dataArrayLeaseNo.length; j++) {
							...
							oLeaseNumberCustPMR.addItem(new sap.ui.core.ListItem({
								text: dataArrayLeaseNo[j],
								key: dataArrayLeaseNo[j],
								additionalText: dataArrayLeaseNo[j]
							}));
						}
	...
	createCustPMRReportTable: function () {
		...
		var oLabelTitle = new sap.ui.commons.Label({
			text: "Redelivery Schedule",
			wrapping: true
		}).addStyleClass("font15Bold marginTop10");

		var oLabelPMR = new sap.ui.commons.Label({
			text: "PMR- Permitted Monthly Returns",
			wrapping: true
		}).addStyleClass("marginTop10");

		var oLabelTRQ = new sap.ui.commons.Label({
			text: "TRQ- Total Returns Quantity",
			wrapping: true
		}).addStyleClass("marginTop10");

		var oLabelSpace = new sap.ui.commons.Label({
			text: " ",
			width: "25px",
			wrapping: true
		}).addStyleClass("marginTop10");
	...
	getPMRReport: function () {
		...
		var urlToCall = serviceUrl + "/Returns_Schedule?$filter=ILeaseNo eq '" + ILeaseNumbers + "' and IProLocation eq '" + ILocation +
			"' and IProLocation1 eq '" + ILocation1 + "' and IRdate eq '" + CurrentDateSend + "' and IPartnerNo eq '" + vCustomer +
			"' and UnitType eq '" + IUnitDescription + "' and IRet1 eq '' and IRet2 eq ''";
		...
		OData.request({
			...
			function (data, response) {
				...
				if (len != 0) {
					...
					oModelPMRReport.setData({
						modelData: aPMRReportData
					});
					...
					if (sap.ui.getCore().byId("idPMRReportTblFlex"))
						sap.ui.getCore().byId("idPMRReportTblFE").destroyFields();
					var msg =
						"No Return Schedule data found for the specified search criteria, please contact your local customer services for any queries.";
