...
sap.ui.model.json.JSONModel.extend("custReportUnitsOnLeaseView", {
	...
	createCustReportUnitsOnLease: function () {
		...
		var oLabelSpaceCRUOL = new sap.ui.commons.Label({
			text: " ",
			width: "8px",
			wrapping: true
		});
	...
	getUnitsOnLeaseReportCRUOL: function () {
		...
		var urlToCallCRUOL = serviceUrl + "/Units_OnLease?$filter=ICustomerId eq '" + customerID + "' and ILeaseType eq '" + leaseTypeString +
			"' and ILeaseNumber eq '" + leaseNumberString + "' and IUnitDesc eq '" + unitDescrsString +
	...
	updateUnitsOnLeaseReportCRUOL: function () {
		...
		oModelUnitsOnLeaseReportCRUOL.setData({
			modelData: arrUnitsOnLeaseReport
		});
