...
sap.ui.model.json.JSONModel.extend("CustDashOutReturnReport", {
	...
	createCustDashOutReturnReport: function () {
		...
		var olblSelCutId = new sap.ui.commons.Label({
			text: "Customer ID:",
			required: true,
		...
		var oLabelUtypeCOD = new sap.ui.commons.Label({
			text: "Unit Type",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelLeaseCOD = new sap.ui.commons.Label({
			text: "Lease",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelCountryCOD = new sap.ui.commons.Label({
			text: "Country",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelCityCOD = new sap.ui.commons.Label({
			text: "City",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelSelRedelId = new sap.ui.commons.Label({
			text: "Redelivery Ref:",
			required: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelSpaceCORetReport = new sap.ui.commons.Label({
			text: " ",
			width: "8px",
			wrapping: true
		});
		...
		var oLabelMandatory = new sap.ui.commons.Label({
			text: "Either of the fields required", // MAC11122017_2
			required: true,
	...
	populateCountryCityCOD: function (customer) {
		...
		OData.request({
			...
			function (data, response) {
				...
				oComboCityCOD.bindItems("/aCityCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
				...
				oComboCountryCOD.bindItems("/aCountryCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
				...
				oComboLeaseCOD.bindItems("/aLeaseCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
				...
				oComboUtypeCOD.bindItems("/aUtypeCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
	...
	getOnlineCutomerList: function () {
		try {
			...
			if (objLoginUser.getLoggedInUserType().toUpperCase() == "SEACO") {
				...
				urlToCall = serviceUrl + "/F4_Multiple_Cust?$filter=Bname eq '" + objLoginUser.getLoggedInUserName().toUpperCase() +
					"' and Customer eq '' and Param1 eq ''";
			}
	...
	getOnlineCutomerListSuccess: function (resultdata, response) {
		...
		try {
			oAutoCmpltSelCustCORetReport.insertItem((new sap.ui.core.ListItem({
				text: "",
				key: "0"
			})), 0);
			...
			if (resultdata != undefined) {
				if (resultdata.results.length > 0) {
					...
					for (var j = 0; j < resultdata.results.length; j++) {
						oAutoCmpltSelCustCORetReport.addItem(new sap.ui.core.ListItem({
							text: resultdata.results[j].CustName,
							key: resultdata.results[j].Partner,
							additionalText: resultdata.results[j].Partner
						}));
					}
	...
	getOnlinesuccessLoadData: function (resultdata, response) {
		...
		if (jsnLoadSavedDataCDORetRpt.length == 1) {
			if ((jsnLoadSavedDataCDORetRpt[0].Lease == '') && ($.trim(jsnLoadSavedDataCDORetRpt[0].Ordno) == '')) {
				...
				var objCurntCustDORR = new CustDashOutReturnReport();
				sap.ui.getCore().byId("idFrmElemntLoadDataCustORetReport").insertField(objCurntCustDORR.createTableLoadSavedData(
					jsnLoadSavedDataCDORetRpt));
			}
	...
	createTableLoadSavedData: function (jsnLoadSavedDataCDORetRpt) {
		var olblTblHdrCustOutRelRep = new sap.ui.commons.Label({
			text: 'Redelivery Summary for ' + sap.ui.getCore().byId("idAutoCmpltSelCustCORetReport").getValue(),
			wrapping: true
		...
		var obtnExportExlCORetReport = new sap.m.Button("idbtnExportExlCORetReport", {
			...
			press: function () {
				...
				var arrDiplHdrExprt = ['Lease', 'Return', 'Unit Description', 'Location', 'Depot', 'Total Quantity', 'Quantity Returned',
					'Outstanding Quantity', 'Expiry Date'
				];
		...
		oMdlLoadDataCDORetRep.setData({
			modelData: jsnLoadSavedDataCDORetRpt
		});
		oidTblRelSummDataCDORetR.setModel(oMdlLoadDataCDORetRep);
		oidTblRelSummDataCDORetR.bindRows("/modelData");
		if (jsnLoadSavedDataCDORetRpt.length < 1) {
	...
	getOnlinesuccessCDORetRptDtl: function (resultdata, response) {
		...
		if (jsnLoadSavedDataCDORetDtlRpt.length > 0) {
			...
			bus.publish("nav", "to", {
				id: "CustOutReturnDtlReportVw",
				data: {
					bindData: jsnLoadSavedDataCDORetDtlRpt,
					relDtlFor: vTblhdr
				}
			});
	...
	getOnlinesuccessCDORetRptDtlNew: function (inputpara) {
		...
		for (var indx in globalLoadSavedDataCDORetRpt) {
			if (globalLoadSavedDataCDORetRpt[indx].Lease == parseInt(datasplt[0]) && globalLoadSavedDataCDORetRpt[indx].Ordno == parseInt(
					datasplt[1]) && globalLoadSavedDataCDORetRpt[indx].UnitType == datasplt[2] && globalLoadSavedDataCDORetRpt[indx].City == datasplt[3]) {
				/*if($.trim(resultdata.results[indx].OffDate) != 'X'){
					var pattern = /(\d{2})\-(\d{2})\-(\d{4})/;
					 dateForSort = new Date(resultdata.results[indx].Extra1.replace(pattern,'$3-$2-$1'));
				}else{
