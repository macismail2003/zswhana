...
sap.ui.model.json.JSONModel.extend("CustDashOutRelReport", {
	...
	createCustDashOutRelReport: function () {
		...
		var oLabelUtypeCOR = new sap.ui.commons.Label({
			text: "Unit Type",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelLeaseCOR = new sap.ui.commons.Label({
			text: "Lease",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelCountryCOR = new sap.ui.commons.Label({
			text: "Country",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelCityCOR = new sap.ui.commons.Label({
			text: "City",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelSelBookingId = new sap.ui.commons.Label({
			text: "Booking Ref:",
			required: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L2 M4 S12",
		...
		var oLabelSpaceCORReport = new sap.ui.commons.Label({
			text: " ",
			width: "8px",
			wrapping: true
		});
			...
			if (resultdata != undefined) {
				if (resultdata.results.length > 0) {
					...
					for (var j = 0; j < resultdata.results.length; j++) {
						oAutoCmpltSelCustCORReport.addItem(new sap.ui.core.ListItem({
	...
	populateCountryCityCOR: function (customer) {
		...
		OData.request({
			...
			function (data, response) {
				...
				oComboCityCOR.bindItems("/aCityCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
				...
				oComboCountryCOR.bindItems("/aCountryCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
				...
				oComboLeaseCOR.bindItems("/aLeaseCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
				...
				oComboUtypeCOR.bindItems("/aUtypeCombo", new sap.ui.core.ListItem({
					text: "{}"
				}));
	...
	createTableLoadSavedData: function (jsnLoadSavedData) {
		var olblTblHdrCustOutRelRep = new sap.ui.commons.Label({
			text: 'Booking Summary for ' + sap.ui.getCore().byId("idAutoCmpltSelCustCORReport").getValue(),
			wrapping: true
		...
		var obtnExportExlCORReport = new sap.m.Button("idbtnExportExlCORReport", {
			...
			press: function () {
				...
				var arrDiplHdrExprtCDBRel = ['Lease', 'Release', 'Unit Description', 'Location', 'Depot', 'Total Quantity', 'Quantity Picked',
					'Outstanding Quantity', 'Expiry Date'
				];
				var arrDiplFieldExprtCDBRel = ['LeaseNo', 'ReleaseAuth', 'UnitDesc', 'Location', 'Depot', 'TotQty', 'PickedQty', 'OutstandQty',
					'ExpiryDate'
				];
	...
	getOnlinesuccessCDORelRptDtlNew: function (inputpara) {
		...
		if (jsnLoadSavedDataCDORelDtlRpt.length > 0) {
			...
			bus.publish("nav", "to", {
				id: "CustOutRelDtlReportVw",
				data: {
					bindData: jsnLoadSavedDataCDORelDtlRpt,
					relDtlFor: vTblhdr
				}
			});
