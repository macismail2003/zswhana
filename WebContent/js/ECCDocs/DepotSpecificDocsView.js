...
sap.ui.model.json.JSONModel.extend("depotSpecDocsView", {
	...
	createDepotSpecDocs: function () {
		...
		var oLabelDocType = new sap.ui.commons.Label("docType", {
			required: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L4 M3 S12"
			}),
		...
		/*temp start*/
		var oLabelDepotCodeDoc = new sap.ui.commons.Label("idLblDepotCodeDoc", {
			required: true,
			text: "Depot Code: ",
			layoutData: new sap.ui.layout.GridData({
				span: "L4 M3 S4"
	...
	getDocType: function () {
		...
		vComboDocType.bindItems("/aDocType", new sap.ui.core.ListItem({
			text: "{DocType}"
		}), new sap.ui.model.Sorter("/aDocType"));
...
*/
	...
	getMailDetails: function (vRdBtnSel) {
		...
		if (valid && valid1) {
			...
			if (valueforDocType[0].Zdoc == 'DN_JS') {
				urlToCall = serviceUrl + "/Sost_Details_Dn?$filter=IDepoCode eq '" + depotCode + "' and IPeriod eq '" + vRdBtnSel +
					"' and IDoctype eq '" + valueforDocType[0].Zdoc + "'";
			} else {
				urlToCall = serviceUrl + "/Sost_Details?$filter=IDepoCode eq '" + depotCode + "' and IPeriod eq '" + vRdBtnSel +
					"' and IDoctype eq '" + valueforDocType[0].Zdoc + "'";
			}
	...
	GetMessage: function (vSelTitle, vSelObj) {
		busyDialog.open();
		var oCurrent = this;
		var oOverlayMail = new sap.ui.ux3.OverlayDialog({
			width: "600px",
			height: "400px"
		});
		...
		oModel = new sap.ui.model.odata.ODataModel(serviceUrl, true);
		var urlToCall = serviceUrl + "/Sost_Email_Body?$filter=Objtp eq '" + vObjTp + "' and Objyr eq '" + vObjYr + "' and Objno eq '" +
			vObjNo + "' and Forno eq '" + vForNo + "' and Fortp eq '" + vForTp + "' and Foryr eq '" + vForYr + "'";
		...
		OData.request({
			...
			function (data, response) {
				...
				var oLabelTitle = new sap.ui.commons.Label({
					...
					wrapping: true
				}).addStyleClass("fontTitle marginTop5");
				...
				var oLabelSubject = new sap.ui.commons.Label({
					text: "Subject : ",
					layoutData: new sap.ui.layout.GridData({
						span: "L2 M2 S2",
						linebreak: false,
				...
				var oLabelAttachment = new sap.ui.commons.Label({
					text: "Attachment : ",
					layoutData: new sap.ui.layout.GridData({
						span: "L2 M2 S2",
						linebreak: false,
	...
	OpenMailAttachment: function (vObjNo, vObjTp, vObjYr, title, isDn) {
		...
		var urlToCall = serviceUrl + "/Sost_Email_Content_Pdf(IForno='',IFortp='',IForyr='',IObjno='" + vObjNo + "',IDn='" + vDn +
			"',IObjtp='" + vObjTp + "',IObjyr='" + vObjYr + "',IFlag='X')";
		//alert("url " + urlToCall);
		OData.request({
				requestUri: urlToCall,
				method: "GET",
