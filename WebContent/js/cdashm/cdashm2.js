...
sap.ui.model.json.JSONModel.extend("CDASHM2", {
	...
	setValueHeaderDetails: function (isFromSerialHistory, isProcessChange) {
			...
			function (data, response) {
				...
				if (isProcessChange == true) { // If this is a process change, don't change the dropdown value
					...
					if (oCDASHM2JsonProcessesLoc != "")
						oCDASHM2JsonProcessesLoc.sort(function (a, b) {
					...
					oCDASHM2ModelRecordDetails.setData({
						modelData: oCDASHM2JsonRecordLines
					});
					...
					if (data.EstimateDate != "") {
						oCDASHM2JsonHeaderLines[0].value1 = data.EstimateDate.substr(0, 2) + " " + m_names[parseInt(data.EstimateDate.substr(3, 2)) - 1] +
					...
					if (data.GateInDate != "") {
						...
						oCDASHM2JsonHeaderLines[1].value1 = data.GateInDate.substr(0, 2) + " " + m_names[parseInt(data.GateInDate.substr(3, 2)) - 1] +
					...
					if (data.CustAppDate != "") {
						...
						oCDASHM2JsonHeaderLines[1].value3 = data.CustAppDate.substr(0, 2) + " " + m_names[parseInt(data.CustAppDate.substr(3, 2)) - 1] +
					...
					if (data.GateOutDate == "") {
						...
						oCDASHM2JsonHeaderLines[2].value1 = data.GateOutDate.substr(0, 2) + " " + m_names[parseInt(data.GateOutDate.substr(3, 2)) - 1] +
		...
		oCDASHM2ModelHeaderDetails.setData({
			modelData: oCDASHM2JsonHeaderLines
		});
	...
	setContentMiscInfo: function () {
		...
		oCDASHM2TableMiscInfo.addColumn(new sap.ui.table.Column({
			...
			template: new sap.ui.commons.TextView({}).bindProperty("text", "labcost").addStyleClass("borderStyle1"),
			resizable: false,
			width: "50px"
		}));
		...
		oCDASHM2TableMiscInfo.addColumn(new sap.ui.table.Column({
			...
			template: new sap.ui.commons.TextView({}).bindProperty("text", "matcost").addStyleClass("borderStyle1"),
			resizable: false,
			width: "50px"
		}));
		...
		oCDASHM2TableMiscInfo.addColumn(new sap.ui.table.Column({
			...
			template: new sap.ui.commons.TextView({}).bindProperty("text", "total").addStyleClass("borderStyle1"),
			resizable: false,
			width: "50px"
		}));
	...
	setValueMiscInfo: function () {
		...
		oCDASHM2ModelMiscInfo.setData({
			modelData: oCDASHM2JsonMiscInfo
		});
		...
		OData.request({
			...
			function (data, response) {
				...
				for (var i = 0; i < oCDASHM2JsonEstimateLinesLoc.length; i++) {
					...
					} else if (oCDASHM2JsonEstimateLinesLoc[i].LabourCost == "Seacover" || oCDASHM2JsonEstimateLinesLoc[i].LabourCost == "Lessee" ||
	...
	setContentThumbnail: function (CDASHM2SerialNo) {
			...
			var oCDASHM2FlexImagePage = new sap.m.FlexBox(oCDASHM2FlexImagePageFlexId, { // // MAC09052018
				...
				template: new sap.ui.layout.VerticalLayout(oCDASHM2FlexImagePageVerticalLayoutId, { // // MAC09052018
					content: [
						...
						new sap.m.Label({
							text: "{dispname}"
						}).addStyleClass("thumbnailimagescaption")
					]
				})
		...
		oCDASHM2ModelOtherDocs.setData({
			modelData: jsonCDASHM2Documents
		});
	...
	setContentHeaderDetailsBackUp: function () {
		...
		var oCDASHM2HeaderDetailsDividerBegin = new sap.ui.commons.HorizontalDivider({
			width: "100%",
			type: "Area",
			height: "Medium"
		});
		var oCDASHM2HeaderDetailsDividerEnd = new sap.ui.commons.HorizontalDivider({
			width: "100%",
			type: "Area",
			height: "Medium"
		});
	...
	sendRequestPictures: function (button) {
		...
		var oCDASHM2RequestPicturesValueBody = new sap.m.TextArea("idCDASHM2RequestPicturesValueBody", {
			...
			value: "Dear Sir/Madam,\n\nPlease submit photos of the unit " + global3SerialNo +
				" by replying to this email with photo@seacoglobal.com in copy." + "\n\n" + "Comment:\n<insert comment here>" + "\n\n" +
				"Please note:\n- Email subject must be in the form of " + global3SerialNo + "_" + global3Depot +
	...
	downloadCDASHM2Excel: function () {
		...
		htmlTable += '<tr style="height:75px;border=1;padding-bottom:10px">' +
			'<td align=center colspan=' + (tableWidth - 12) + ' style="padding-left:10px;font:bold 14px Arial;"><u>' +
			"Unit Overview for Serial No. " + ' for <b><FONT COLOR="RED">' + global3SerialNo + ' </FONT></b> in depot <b><FONT COLOR="RED">' +
			global3Depot + '</FONT></b></u></td>' +
		...
		*/
		...
		if (func == "print") {
			...
			var ctx = {
				worksheet: "M&R Unit Overview" || 'Worksheet',
				table: htmlTable
			};
