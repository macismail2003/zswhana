...
sap.ui.model.json.JSONModel.extend("depInvRepairView", {
	...
	createDepInvRepairFormView: function () {
		...
		oDepInvRepTable.addColumn(new sap.ui.table.Column("idChkBoxColDRV", {
		...
		// Labels
		var oLabelInfo = new sap.ui.commons.Label({
			text: "Click on Unit Number to see more information. ",
			wrapping: true
		}).addStyleClass("margin10");

		// Table - UNITS NOT FOUND
		var oLabelUnitsNF = new sap.ui.commons.Label({
			text: "Units Not Found ",
			wrapping: true
		}).addStyleClass("font15Bold marginStyle");
		...
		oModelDRVUnitNotFound.setData({
			modelData: aUnitsNotFoundDRV
		});
		...
		// Labels
		var oLabelUnitsOnLease = new sap.ui.commons.TextView({
			text: "Units Currently not on lease \n with " + vCustomerDRV,
			wrapping: true
		}).addStyleClass("font15Bold marginStyle");
		...
		oModelNotOnLeaseDRV.setData({
			modelData: aUnitsNotOnLeaseDRV
		});
		...
		var oLabelTbltitle = new sap.ui.commons.TextView({
			text: "Results for Depreciated Replacement Value (DRV) " + currentDate,
			layoutData: new sap.ui.layout.GridData({
				span: "L12 M12 S4",
				linebreak: true,
				margin: false
		...
		var backDRVResults = new sap.m.Link("idBackDRVResults", {
			text: " < Back",
			width: "7%",
			wrapping: true,
			press: function () {
	...
	bindRepairStatus: function () {
		...
		if (aLen == 0) {
			...
			var oModelDRVResult = new sap.ui.model.json.JSONModel();
			oModelDRVResult.setData({
				modelData: aDRVDetails
			});
		...
		for (var j = 0; j < aDRVDetails.length; j++) {
			for (var i = 0; i < vChkBoxChkdLen; i++) {
				if (aDRVDetails[j].Sernr == aChkBoxChkd[i].name) {
					if (count == 0) {
						Row[count] = aDRVDetails[j].Sernr + "|" + aDRVDetails[j].UnitType + "|" + aDRVDetails[j].Lease + "|" + aDRVDetails[j].Rate.trim() +
							"|" + aDRVDetails[j].Currency + "|" + aDRVDetails[j].DepotComments + "@" + vUserName;
					} else {
						Row[count] = aDRVDetails[j].Sernr + "|" + aDRVDetails[j].UnitType + "|" + aDRVDetails[j].Lease + "|" + aDRVDetails[j].Rate.trim() +
							"|" + aDRVDetails[j].Currency + "|" + aDRVDetails[j].DepotComments;
					}
					count++;
					break;
				}
