...
sap.ui.model.json.JSONModel.extend("utility", {
			...
			function (oEvent) {
				...
				if ((pageidlanding != '') && (pageidlanding != 'WelcomePg')) {
					...
					for (var indx in nodesinTree) {
						if (((nodesinTree[indx].sId == oEvent.getParameter('node').sId) || (nodesinTree[indx].sId == crntSelMenuParent.sId)) && (oEvent.getParameter(
								'node').hasChildren()) && (!oEvent.getParameter('node').getExpanded())) {
	...
	createScreenMenufromJSON: function (data, oCurrent) {
		...
		var labUserName = new sap.ui.commons.Label({
			text: userName,
			wrapping: true,
	...
	makeHTMLTableEDI: function (json, title, btnTxt, depotTxt) {
		...
		htmlTable += '<tr style="height:75px;">' +
			...
			'<td style="border:none; padding:5px 5px 5px 0px" colspan=1 align ="right"><img src="' + base +
			'images/login/login_logo.png"></img></td></tr>' + '<tr><td colspan=5 style="border:none;padding-left:10px;font:bold 15px Arial;">' +
			depotTxt + '</td></tr>';
		htmlTable += "<tr>";
	...
	ExportUtility: function (JsnToExport, displayClmnHdrNamArr, displayClmnFildArr, title, btnTxt) {
		...
		htmlTable += '<table border=1 cellspacing=0 style="color:#656465">';
		htmlTable += '<tr style="height:75px;">' +
			'<td colspan=' + (displayClmnHdrNamArr.length - 1) + ' valign="middle" style="border:none;padding-left:10px;font:bold 18px Arial;">' +
			title + '</td>' +
			'<td style="border:none; padding:5px 5px 5px 0px" colspan=1 align ="right"><img src="' + base +
			'images/login/login_logo.png"></img></td></tr>';
		htmlTable += "<tr>";
	...
	makeHTMLTableEST: function (depotForPrint,
		...
		for (var i = 0; i < excelDataPrint.length; i++) {
			if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'U') {
				labhrU = (excelDataPrint[i][9] == "") ? labhrU : parseFloat(parseFloat(labhrU).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[9]).toFixed(2));
				matrcU = (excelDataPrint[i][10] == "") ? matrcU : parseFloat(parseFloat(matrcU).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[10]).toFixed(2));
			} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'J') {
				labhrJ = (excelDataPrint[i][9] == "") ? labhrJ : parseFloat(parseFloat(labhrJ).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[9]).toFixed(2));
				matrcJ = (excelDataPrint[i][10] == "") ? matrcJ : parseFloat(parseFloat(matrcJ).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[10]).toFixed(2));
			} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'O') {
				labhrO = (excelDataPrint[i][9] == "") ? labhrO : parseFloat(parseFloat(labhrO).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[9]).toFixed(2));
				matrcO = (excelDataPrint[i][10] == "") ? matrcO : parseFloat(parseFloat(matrcO).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[10]).toFixed(2));
			} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'V') {
				labhrV = (excelDataPrint[i][9] == "") ? labhrV : parseFloat(parseFloat(labhrV).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[9]).toFixed(2));
				matrcV = (excelDataPrint[i][10] == "") ? matrcV : parseFloat(parseFloat(matrcV).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
					[10]).toFixed(2));
			}
		}
		...
		var labrt = parseFloat(parseFloat(labrtU).toFixed(2)) + parseFloat(parseFloat(labrtJ).toFixed(2)) + parseFloat(parseFloat(labrtO).toFixed(
				2)) +
		...
		htmlTable += '<tr style="height:75px;border=1;padding-bottom:10px">' +
			'<td align=center colspan=' + (tableWidth - 12) + ' style="padding-left:10px;font:bold 14px Arial;"><u>' + title +
			' for <b><FONT COLOR="RED">' + serialForPrint + ' </FONT></b> in depot <b><FONT COLOR="RED">' + depotForPrint +
			'</FONT></b> on <b><FONT COLOR="RED">' + estimDateForPrint + '</FONT></b></u></td>' +
		...
		if (estimDateForPrint != '')
			htmlTable += '<td align=left style="width:250px;padding-left:10px; font: 12px Arial;">Estim Date : <b>' + estimDateForPrint +
			'</b></td>';
		...
		if (oFESTInputPOHDate != '')
			htmlTable += '<td align=left style="width:250px;padding-left:10px; font: 12px Arial;">Prev. On Hire Date : <b>' + oFESTInputPOHDate +
			'</b></td>';

		if (oFESTInputPOHLocation != '')
			htmlTable += '<td align=left  style="padding-left:10px; font: 12px Arial;">Prev. On Hire Loc : <b>' + oFESTInputPOHLocation +
			'</b></td>';
...
function errorfromServer(err) {
	...
	if (err.response.statusCode == "500") {
		sap.ui.commons.MessageBox.show("Your request could not be processed at this time.\n Please try again later.",
			...
			"Information", [sap.ui.commons.MessageBox.Action.OK],
...
function CheckIdleTime() {
	...
	if (_idleSecondsCounter >= IDLE_TIMEOUT && !TimeoutFlag) {
		...
		sap.ui.commons.MessageBox.show("Session expired.",
			...
			"Warning", [sap.ui.commons.MessageBox.Action.OK],
