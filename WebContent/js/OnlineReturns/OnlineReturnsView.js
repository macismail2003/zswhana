...
sap.ui.model.json.JSONModel.extend("onlineRetView", {
	...
	createOnlineRet: function () {
		...
		var oCurrent = this;
		var labStar = new sap.ui.commons.Label({
			text: "* "
		}).addStyleClass("MandatoryStar");
		var lblSpaceLegend = new sap.ui.commons.Label({
			text: " ",
			width: '5px'
		});
		var labRequired = new sap.ui.commons.Label({
    ...
    */
		...
		var lblSpace1 = new sap.ui.commons.Label({
			text: " ",
			width: '8px'
		});
...
*/
		// Labels
		var oLabelCustomer = new sap.ui.commons.Label({
			text: "Customer: ",
			required: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
		...
		var oLabelRetDate = new sap.ui.commons.Label({
			text: "Start Date: ",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
		...
		var oLabelExpDate = new sap.ui.commons.Label({
			text: "Expiry Date: ",
			required: false,
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
		...
		var oLabelUnitNo = new sap.ui.commons.Label({
			text: "Unit Number: ",
			required: true,
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
		...
		var oLabelWarning = new sap.ui.commons.Label({
			text: "For individual containers, please enter the 4 letter prefix and 7 digit number for each container. (Eg. SEGU3000511). For multiple containers (Max 25), press the Enter key after each one," +
				" or copy and paste the list to the entry field",
			wrapping: true
		}).addStyleClass("font10");
		var LblFlex = new sap.m.FlexBox({
			items: [oLabelUnitNo, oLabelWarning],
			direction: "Column",
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
				linebreak: true,
	...
	getUnitsToBrowse: function () {
		...
		var customerOR = "";
		if (vSelCustomerOR == "" || vSelCustomerOR == "Select Customer" || vSelContractOR.getValue() == "" || vSelContractOR.getValue() ==
			"Select Contract Number" || vSelUnitTypeOR.getValue() == "" || vSelUnitTypeOR.getValue() == "Select Unit Type" ||
			vSelCou +
			...
			var urlToCall = serviceUrl + "/Return_Browse?$filter=ICustomer eq '" + customerOR + "' and IContractNo eq '" + vSelContractOR.getValue() +
				"' and IUnitType eq '" + vSelUnitTypeOR.getValue() + "'  and ICountry eq '" + vSelCountryOR.val() + "' and ICity eq '" + vSelCityOR.val() +
	...
	createCurrRetTable: function () {
		...
		var lblSpaceMessageTxt = new sap.ui.commons.Label({
			text: "Please delete all the units with status ERROR to SUBMIT.",
			wrapping: true
		}).addStyleClass("WarningMessage");
		...
		var oLabelCurrRetTblTitle = new sap.ui.commons.Label({
			text: "Redelivery Requests",
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
				linebreak: true,
				margin: false
	...
	createNBindCurrentReturns: function () {
		...
		if (valid) {
			if (oCurrent.checkLimit25OR()) {
				...
				if (unitNumbrsEntered > 0) {
					var presentUnitnumbers =
						"The following unit numbers already exist in the Current Returns Section.\nPlease remove the duplicate Unit Number(s) and re-try.\n" +
				...
				oModelCurrReturns.setData({
					modelData: aCurrentReturnsOR
				});
				...
				var Limit25Message =
					"Each Return Authorization (RA) can contain maximum 25 units. \n To return more than 25 units, please create multiple RA's";
	...
	ResetORForm: function () {
		...
		sap.ui.commons.MessageBox.show(ConfirmMessage,
			sap.ui.commons.MessageBox.Icon.WARNING,
			"Warning", [sap.ui.commons.MessageBox.Action.YES, sap.ui.commons.MessageBox.Action.NO],
			...
			for (var j = 0; j < aCurrentReturnsOR.length; j++) {
				for (var i = 0; i < len; i++) {
					...
					if (aCurrentReturnsOR[j].SerialNo == aChkBoxCurrRetValidateOR[i].SerialNo) {
						...
						Row[count] = customerOR + "$$$$" + aCurrentReturnsOR[j].Location + "$$" + aCurrentReturnsOR[j].SerialNo.toUpperCase() + "$" +
				...
				function (data, response) {
					...
					for (var k = 0; k < aCurrentReturnsOR.length; k++) {
						...
						if (errArray.length > 0) {
							...
							if (aCurrentReturnsOR[k].ErrMsgno == "999" && aCurrentReturnsOR[k].ErrLine == "9999" && aCurrentReturnsOR[k].Status.trim().toUpperCase() ==
								"ERROR") {
								aCurrentReturnsOR[k].Status = errArray[0].Status;
								aCurrentReturnsOR[k].isStatusEnabled = true;
							}
			...
			function (data, response) {
				if (data.results.length > 0) {
					...
					if (data.results[0].ErrType == "S") {
						...
						var htmlContentRaNo =
							'<button type="button" id="idBtnRaNo" data-sap-ui="idBtnRaNo" role="button" aria-disabled="true" tabindex="0" style="width:280px" class="redelReference">' +
							raSuccessMsg + '</button>';
