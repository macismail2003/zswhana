...
sap.ui.model.json.JSONModel.extend("raSearchView", {
	...
	createRASearchView: function () {
		...
		var vHDivider1 = new sap.ui.commons.HorizontalDivider({
			width: "95%",
			type: "Area",
			height: "Medium"
		});
	...
	getRADetails: function () {
		...
		var urlToCall = serviceUrl + "/Edit_Returns_Detail?$filter=Ordno eq '" + vRANumber + "' and Customer eq '" + vCust +
			"' and Extra1 eq '" + vUserName + "'";
				...
				if (len > 0) {
					if (data.results[0].Extra1.indexOf("different customer.") != -1) {
						...
						for (var i = 0; i < len; i++) {
							...
							var helpToPass = data.results[i].Ordno + "$" + data.results[i].UnitType + "$" + data.results[i].Location + "$" + data.results[i]
								.Depot;
						...
						oModelReturnResultDetails.setData({
							modelData: aEDRetResultData
						});
	...
	createResultTableEDRetResult: function () {
		...
		var oLabelTblTitle = new sap.ui.commons.Label({
			text: "Redelivery Details",
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M3 S4",
				linebreak: true,
				margin: false
		...
		var lblSpace = new sap.ui.commons.Label("idSpcForCancelRA", {
			text: " ",
			width: '5px'
		}).addStyleClass("marginTop10");
