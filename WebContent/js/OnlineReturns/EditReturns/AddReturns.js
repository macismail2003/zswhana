...
sap.ui.model.json.JSONModel.extend("EDAddReturnView", {
	...
	createAddReturnView: function () {
		var oCurrent = this;
		var backRetDetsDetOR = new sap.m.Link({
			text: " < Back",
			width: "15%",
			wrapping: true,
			press: function () {
		...
		var oLabelUnitNumber = new sap.ui.commons.Label({
			text: "Unit Number",
		...
		var oLabelWarning = new sap.ui.commons.Label({
			text: "For individual containers, please enter the 4 letter prefix and 7 digit number for each container. (Eg. SEGU3000511). For multiple containers (Max 25), press the Enter key after each one, or copy and paste the list to the entry field",
			wrapping: true
		...
		var LblFlex = new sap.m.FlexBox({
			items: [oLabelUnitNumber, oLabelWarning],
			direction: "Column",
			layoutData: new sap.ui.layout.GridData({
				span: "L3 M4 S5",
				linebreak: true,
		...
		//city drop down
		var oComboCity = new sap.ui.core.HTML("idHtmlCityEDRetAdd", {
			layoutData: new sap.ui.layout.GridData({
				span: "L6 M5 S12"
			})
		});
		var htmlContentCity =
			'<input id="idComboCityEDRetAdd" placeholder="Select City" class="FormInputStyle marginTop10" style="width:100%">';
		...
		var lblSpaceBtn = new sap.ui.commons.Label({
			text: "",
			width: "8px",
			wrapping: true
		});
	...
	createCurrRetTableEDRet: function () {
		...
		var lblSpaceMessageTxt = new sap.ui.commons.Label({
			text: "Please delete all the units with status ERROR to SAVE.",
			wrapping: true
			...
			for (var j = 0; j < aAddRetTblData.length; j++) {
				for (var i = 0; i < len; i++) {
					...
					if (aAddRetTblData[j].SerialNo == aChkBoxCurrRetValidateEDRet[i].SerialNo) {
						...
						Row[count] = oUtil.removeLeadZero(vCustomerIDEDRet) + "$$$$" + aAddRetTblData[j].Location + "$$" + aAddRetTblData[j].SerialNo.toUpperCase() +
				...
				function (data, response) {
					...
					for (var k = 0; k < aAddRetTblData.length; k++) {
						...
						if (errArray.length > 0) {
							...
							if (aAddRetTblData[k].ErrMsgno == "999" && aAddRetTblData[k].ErrLine == "9999" && aAddRetTblData[k].Status.trim().toUpperCase() ==
								"ERROR") {
								aAddRetTblData[k].Status = errArray[0].Status;
								aAddRetTblData[k].isStatusEnabled = true;
							}
	...
	submitCurrRetEDRet: function (indexStart, indexEnd) {
		...
		var arrFieldEDRet = ["ReturnData1", "ReturnData2", "ReturnData3", "ReturnData4", "ReturnData5", "ReturnData6", "ReturnData7",
			"ReturnData8", "ReturnData9", "ReturnData10", "ReturnData11", "ReturnData12", "ReturnData13", "ReturnData14", "ReturnData15"
		];
		var arrFieldEDRetMail = ["CtMailData1", "CtMailData2", "CtMailData3", "CtMailData4", "CtMailData5", "CtMailData6", "CtMailData7",
			"CtMailData8", "CtMailData9", "CtMailData10", "CtMailData11", "CtMailData12", "CtMailData13", "CtMailData14", "CtMailData15"
		];
		var urlToCall = "";
