...
sap.ui.model.json.JSONModel.extend("EDReturnDetailsView", {
	...
	createEDReturnDetailsView: function () {
		...
		var oLabelSpace1 = new sap.ui.commons.Label("idLblSpcEDRet1", {
			text: "",
			width: "8px",
			wrapping: true
		...
		var oLabelSpace2 = new sap.ui.commons.Label({
			text: "",
			width: "8px",
			wrapping: true
		}).addStyleClass("marginTop10");
			...
			function (data, response) {
				...
				if (len > 0) {
					...
					for (var i = 0; i < len; i++) {
						...
						var helpValueDetsDEt = data.results[i].Ordno + "$" + data.results[i].UnitType + "$" + data.results[i].Location + "$" + data.results[
							i].Depot;
					...
					oModelReturnDetailsDet.setData({
						modelData: EDReturnsDetailsDetail
					});
	...
	fnCallbackDeleteUnitsEDRet: function (sResult) {
		...
		if (sResult == "YES") {
			...
			urlToCallMailDeltEDRet = serviceUrl + "/Delete_Returns_Mail?$filter=Customer eq '" + vCustomerIDEDRet + "' and ReturnAuth eq '" +
				vRANumber + "' and";
	...
	deleteUnitFromRA: function () {
		...
		if (aDeleteUnitsEDRet.length == 0) {
			...
			var ConfirmMessage =
				"This action will delete the selected Unit Number(s) from the Return Authorization, and cannot be reversed. Do you want to continue?";
