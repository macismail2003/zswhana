...
jsnLineItemREEntryJ.removeValue = function (name, value) {
	...
	sap.ui.model.json.JSONModel.extend("RepairEstimateEntryJ", {
		createRepairEstimateEntryForm: function () {
			...
			//FIRST ROW
			var olblUnitPCodeREEntryJIn = new sap.ui.commons.Label({
				text: "Unit Part Code:",
				required: true,
			...
			oDdlUpcModelIn.setData({
				data: ddlUpcDataIn
			});

			oDdlUnitPCodeREEntryJIn.setModel(oDdlUpcModelIn);
			oDdlUnitPCodeREEntryJIn.setSelectedKey(ddlUpcDataIn[0].key);
			oDdlUnitPCodeREEntryJIn.bindItems("/data", new sap.ui.core.ListItem({
				text: "{text}",
				key: "{key}"
			}));

			var olblJobTypeREEntryJ = new sap.ui.commons.Label({
				text: "Job Type:",
				required: true,
			...
			var oDdlJobTypeREEntryJ = new sap.ui.commons.DropdownBox("idDdlJobTypeREEntryJ", {
				...
				change: function (evnt) {
					...
					if (this.getSelectedKey() == 'Joint Survey' || this.getSelectedKey() == 'Lessor Survey') { // MACALPHA19032018_7+ added Lessor Survey
						...
						var urlToCall = serviceUrl15_old + "/Serialno_Valid_Repair_Estim?$filter=FunctLoc eq '" + depottxt.substr(0, 15) +
							"' and SerialNo eq '" + unitno + "' and Unitpcode eq 'C'";
			...
			var olblSaleGradeREEntryJ = new sap.ui.commons.Label({
				text: "Sale Grade:",
				//required: true,
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
				wrapping: false
			...
			//SECOND ROW
			var olblEstimatDtREEntryJ = new sap.ui.commons.Label({
				text: "Estimate Date:",
				required: true,
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
				wrapping: false
			...
			oMdlDtPkrEstmtREEntryJ.setData({
				dateValue: new Date()
			});
			...
			var olblLeaseAuthAmtREEntryJ = new sap.ui.commons.Label({
				text: "Lessee Authorised Amount:",
				//required: true,
				visible: false,
				layoutData: new sap.ui.layout.GridData({
					span: "L2 M4 S12",
			...
			var olblCurrencyCodeREEntryJ = new sap.ui.commons.Label({
				text: "Currency Code:",
				//required: true,
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
				wrapping: false
			...
			var olblCWREEntryJ = new sap.ui.commons.Label({
				text: "Cargo Worthy:",
				required: true,
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
				wrapping: false
			...
			/* Add SCR Limit */
			var olblNotes1REEntryJ = new sap.ui.commons.Label({
				text: "SCR Limit:",
				//required: true,
				width: "90px",
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
			...
			/* Add Notes 2 */
			var olblNotes2REEntryJ = new sap.ui.commons.Label({
				text: "Notes 2:",
				required: false,
				width: "90px",
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
			...
			/* Add Notes 3 */
			var olblNotes3REEntryJ = new sap.ui.commons.Label({
				text: "Notes 3:",
				required: false,
				width: "90px",
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
			...
			/* Add Notes 4 */
			var olblNotes4REEntryJ = new sap.ui.commons.Label({
				text: "Notes 4:",
				required: false,
				width: "90px",
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
			...
			/* Add Notes 5 */
			var olblNotes5REEntryJ = new sap.ui.commons.Label({
				text: "Notes 5:",
				required: false,
				width: "90px",
				//layoutData: new sap.ui.layout.GridData({span: "L2 M4 S12",linebreak: false, margin: true}),
			...
			var oFlxRepNotesHeader = new sap.m.FlexBox({
				items: [
					olblTopTextREEntryJ, new sap.ui.commons.Label({
						text: "Repair Header",
						layoutData: new sap.ui.layout.GridData({
							span: "L12 M12 S12"
						}),
						wrapping: true
					}).addStyleClass("marginTop7 fontTitle"),
			...
			var olblhdrTblREEntryJ1 = new sap.ui.commons.Label("idlblhdrTblREEntryJ1", {
				text: "Repair Lines for Carcass - Labour Rate : " + globalLabCostC + " " + globalLabCostCurrency, // MACALPHA19032018_5 +
				layoutData: new sap.ui.layout.GridData({
					span: "L12 M12 S12"
				}),
				wrapping: true
			}).addStyleClass("marginLeft20 marginTop7 fontTitle");

			var olblhdrTblREEntryJ2 = new sap.ui.commons.Label("idlblhdrTblREEntryJ2", {
				text: "Repair Lines for Machinery - Labour Rate : " + globalLabCostM + " " + globalLabCostCurrency, // MACALPHA19032018_5 +
				layoutData: new sap.ui.layout.GridData({
					span: "L12 M12 S12"
				}),
				wrapping: true
		...
		createEnterLineDataLay: function (oController) {
			...
			var oBtnBackUnitSelREEntryJ = new sap.m.Button("idBtnBackUnitSelREEntryJ", {
				...
				press: function () {
					...
					sap.ui.commons.MessageBox.show("You will lose any unsaved data.\nDo you want to save before going back?", sap.ui.commons.MessageBox
						.Icon.WARNING, "Warning", [sap.ui.commons.MessageBox.Action.YES, sap.ui.commons.MessageBox.Action.NO, sap.ui.commons.MessageBox
							.Action.CANCEL
						],
			...
			var oFlxBtnSubmitBtnREEntryJ = new sap.m.FlexBox({
				items: [oBtnBackUnitSelREEntryJ, new sap.ui.commons.Label({
						text: " ",
						width: '8px'
					}),
					oBtnSaveEstimatesREEntryJ, new sap.ui.commons.Label({
						text: " ",
						width: '8px'
					}),
					oBtnSubmitREEntryJ, new sap.ui.commons.Label({
						text: " ",
						width: '8px'
					}),
					oBtnClearMessagesREEntryJ, new sap.ui.commons.Label({
						text: " ",
						width: '8px'
					}),
					oBtnPrintREEntry, new sap.ui.commons.Label({
						text: " ",
						width: '8px'
					}), // MACALPHA19032018_9
			...
			if (part == "C") {
				...
				for (var i = 0; i < excelDataJX.length; i++) {
					...
					if (datainsert) {
						...
						if (excelDataJX[i][0] == '' || excelDataJX[i][1] == '' || excelDataJX[i][2] == '' || excelDataJX[i][4] == '' || excelDataJX[i][8] ==
							'' || excelDataJX[i][11] == '' || excelDataJX[i][11] == '' ||
							excelDataJX[i][0] == null || excelDataJX[i][1] == null || excelDataJX[i][2] == null || excelDataJX[i][4] == null || excelDataJX[i]
							[8] == null || excelDataJX[i][9] == null || excelDataJX[i][11] == null) {
							isEmpty = true;
						}
						if (isEmpty == false) {
			...
			*/
			if (isEmpty == true) {
				sap.ui.commons.MessageBox.alert(
					"Lab. Hrs, Material Cost, Location, Component, Damage, Repair & Responsibility cannot be blank. Please fill them up");
			} else {
		...
		onlinesuccessfunLocation: function (resultdata, response) {
			...
			if (resultdata != undefined) {
				if (resultdata.results.length > 0) {
					var oidDdlLocationREEntryJ = sap.ui.getCore().byId("idDdlLocationREEntryJ");
					oidDdlLocationREEntryJ.insertItem(new sap.ui.core.ListItem({
						text: "",
						key: ""
					}), 0);
		...
		onlinefunComponentNew: function (secId) {
			var oidDdlComponentREEntryJ = sap.ui.getCore().byId("idDdlComponentREEntryJ");
			oidDdlComponentREEntryJ.insertItem(new sap.ui.core.ListItem({
				text: "",
				key: ""
			}), 0);
		...
		onlinefunMatrialCodeNew: function () {
			...
			oidDdlMaterialREEntryJ.insertItem(new sap.ui.core.ListItem({
				text: "",
				key: ""
			}), 0);
		...
		onlinesuccessfunRepair: function (resultdata, response) {
			...
			if (resultdata != undefined) {
				if (resultdata.results.length > 0) {
					var oidDdlRepairREEntryJ = sap.ui.getCore().byId("idDdlRepairREEntryJ");
					oidDdlRepairREEntryJ.insertItem(new sap.ui.core.ListItem({
						text: "",
						key: ""
					}), 0);
					for (var inx in resultdata.results) {
		...
		buildIdDown: function (id, newCol) {
			...
			row = row + 1;
			newId = newId.substring(0, (newId.indexOf("-col") + 4)) + 2 + newId.substring(newId.indexOf("-row"), (newId.indexOf("-row") + 4)) +
				row; // Column
			return newId;
		},
		...
		buildId: function (id, field, row, column, newField, nextRow) {
			...
			if (typeof nextRow !== "undefined") {
				if (newField < 11)
					newId = newId.substring(0, newId.indexOf("field") + 5) + 0 + newId.substring(newId.indexOf("-col"), 12) + column + newId.substring(
						newId.indexOf("-row"), newId.length); // Column
				else
					newId = newId.substring(0, newId.indexOf("field") + 5) + 0 + newId.substring(newId.indexOf("-col"), 13) + column + newId.substring(
						newId.indexOf("-row"), newId.length); // Column
			}
			...
			if (typeof column !== "undefined") {
				...
				if (newField < 11)
					newId = newId.substring(0, newId.indexOf("field") + 5) + newField + newId.substring(newId.indexOf("-col"), 12) + column + newId.substring(
						newId.indexOf("-row"), newId.length); // Column
				else
					newId = newId.substring(0, newId.indexOf("field") + 5) + newField + newId.substring(newId.indexOf("-col"), 13) + column + newId.substring(
						newId.indexOf("-row"), newId.length); // Column
			}
		...
		makeHTMLTableLANew: function (
			...
			htmlTable += '<tr style="height:75px;border=1">' +
				'<td align=center colspan=' + (12) + ' style="padding-left:10px;font:bold 14px Arial;"><u>' + title + ' for <b><FONT COLOR="RED">' +
				serialForPrint + ' </FONT></b> in depot <b><FONT COLOR="RED">' + depotForPrint + '</FONT></b> on <b><FONT COLOR="RED">' +
				estimDateForPrint + '</FONT></b></u></td>' +
			...
			if (currencyCodeForPrint != '')
				htmlTable += '<td colspan="2" align=left  style="padding-left:30px; font: 12px Arial;">Curr : <b>' + currencyCodeForPrint +
				'</b></td>';
			...
			if (notes1forprint != '')
				htmlTable += '<td colspan="2" align=left  style="padding-left:30px; font: 12px Arial;">SCR Limit : <b>' + notes1forprint +
				'</b></td>';
			...
			if (printExcelDataC.length > 0) {
				...
				for (var i = 0; i < excelDataPrint.length; i++) {
					if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'U') {
						labhrU = (excelDataPrint[i][8] == "") ? labhrU : parseFloat(parseFloat(labhrU).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[8]).toFixed(2));
						matrcU = (excelDataPrint[i][9] == "") ? matrcU : parseFloat(parseFloat(matrcU).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[9]).toFixed(2));
					} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'J') {
						labhrJ = (excelDataPrint[i][8] == "") ? labhrJ : parseFloat(parseFloat(labhrJ).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[8]).toFixed(2));
						matrcJ = (excelDataPrint[i][9] == "") ? matrcJ : parseFloat(parseFloat(matrcJ).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[9]).toFixed(2));
					} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'O') {
						labhrO = (excelDataPrint[i][8] == "") ? labhrO : parseFloat(parseFloat(labhrO).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[8]).toFixed(2));
						matrcO = (excelDataPrint[i][9] == "") ? matrcO : parseFloat(parseFloat(matrcO).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[9]).toFixed(2));
					} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'I') {
						labhrI = (excelDataPrint[i][8] == "") ? labhrI : parseFloat(parseFloat(labhrI).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[8]).toFixed(2));
						matrcI = (excelDataPrint[i][9] == "") ? matrcI : parseFloat(parseFloat(matrcI).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[9]).toFixed(2));
					} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'V') {
						labhrV = (excelDataPrint[i][8] == "") ? labhrV : parseFloat(parseFloat(labhrV).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[8]).toFixed(2));
						matrcV = (excelDataPrint[i][9] == "") ? matrcV : parseFloat(parseFloat(matrcV).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[9]).toFixed(2));
					} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'S') {
						labhrS = (excelDataPrint[i][8] == "") ? labhrS : parseFloat(parseFloat(labhrS).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[8]).toFixed(2));
						matrcS = (excelDataPrint[i][9] == "") ? matrcS : parseFloat(parseFloat(matrcS).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[i]
							[9]).toFixed(2));
					}
				}
				...
				htmlTable +=
					'<div style="font-size: 20px;text-decoration: underline;padding-top: 10px;padding-right: 10px;padding-bottom: 10px;color: #cc292b;"><b>' +
					sap.ui.getCore().byId("idlblhdrTblREEntryJ1").getText() + '</b></div>';
			...
			if (globalEstimateIsReefer) {
				...
				if (printExcelDataM.length > 0) {
					...
					for (var i = 0; i < excelDataPrint.length; i++) {
						if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'U') {
							labhrU = (excelDataPrint[i][8] == "") ? labhrU : parseFloat(parseFloat(labhrU).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][8]).toFixed(2));
							matrcU = (excelDataPrint[i][9] == "") ? matrcU : parseFloat(parseFloat(matrcU).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][9]).toFixed(2));
						} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'J') {
							labhrJ = (excelDataPrint[i][8] == "") ? labhrJ : parseFloat(parseFloat(labhrJ).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][8]).toFixed(2));
							matrcJ = (excelDataPrint[i][9] == "") ? matrcJ : parseFloat(parseFloat(matrcJ).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][9]).toFixed(2));
						} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'O') {
							labhrO = (excelDataPrint[i][8] == "") ? labhrO : parseFloat(parseFloat(labhrO).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][8]).toFixed(2));
							matrcO = (excelDataPrint[i][9] == "") ? matrcO : parseFloat(parseFloat(matrcO).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][9]).toFixed(2));
						} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'I') {
							labhrI = (excelDataPrint[i][8] == "") ? labhrI : parseFloat(parseFloat(labhrI).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][8]).toFixed(2));
							matrcI = (excelDataPrint[i][9] == "") ? matrcI : parseFloat(parseFloat(matrcI).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][9]).toFixed(2));
						} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'V') {
							labhrV = (excelDataPrint[i][8] == "") ? labhrV : parseFloat(parseFloat(labhrV).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][8]).toFixed(2));
							matrcV = (excelDataPrint[i][9] == "") ? matrcV : parseFloat(parseFloat(matrcV).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][9]).toFixed(2));
						} else if (excelDataPrint[i][11].substr(0, 1).toUpperCase() == 'S') {
							labhrS = (excelDataPrint[i][8] == "") ? labhrS : parseFloat(parseFloat(labhrS).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][8]).toFixed(2));
							matrcS = (excelDataPrint[i][9] == "") ? matrcS : parseFloat(parseFloat(matrcS).toFixed(2)) + parseFloat(parseFloat(excelDataPrint[
								i][9]).toFixed(2));
						}
					}
					...
					labrt = parseFloat(parseFloat(labrtU).toFixed(2)) + parseFloat(parseFloat(labrtO).toFixed(2)) + parseFloat(parseFloat(labrtI).toFixed(
						2)) + parseFloat(parseFloat(labrtS).toFixed(2));
					matrc = matrcU + matrcO + matrcI + matrcS;
					...
					htmlTable +=
						'<div style="font-size: 20px;text-decoration: underline;padding-top: 10px;padding-right: 10px;padding-bottom: 10px;color: #cc292b;"><b>' +
						sap.ui.getCore().byId("idlblhdrTblREEntryJ2").getText() + '</b></div>';
					htmlTable += '<br>';
			...
			// Total Section
			htmlTable +=
				'<div style="font-size: 20px;text-decoration: underline;padding-top: 10px;padding-right: 10px;padding-bottom: 10px;color: #cc292b;"><b>Total</b></div>';
			htmlTable += '<br>';
			var toal = "";
			var printTotalCost = [];
			labrtU = parseFloat(labrtU).toFixed(2);
		...
		saveRepairEstimateFromExcelNew: function () {
			...
			for (var i = 0; i < excelDataJX.length; i++) {
				...
				if (datainsert) {
					if (excelDataJX[i][0] == '' || excelDataJX[i][1] == '' || excelDataJX[i][2] == '' || excelDataJX[i][4] == '' || excelDataJX[i][8] ==
						'' || excelDataJX[i][9] == '' || excelDataJX[i][11] == '' ||
						excelDataJX[i][0] == null || excelDataJX[i][1] == null || excelDataJX[i][2] == null || excelDataJX[i][4] == null || excelDataJX[i][
							8
						] == null || excelDataJX[i][9] == null || excelDataJX[i][11] == null) {
						isEmpty = true;
					}
					if (isEmpty == false) {
		...
		checkIfDropDownsAreValid: function () {
			...
			for (var i = 0; i < saveLineItemREEntryJCount; i++) {
				...
				if (saveLineItemREEntryJ[i].MaterialKey != null && saveLineItemREEntryJ[i].MaterialKey != "") {
		...
		checkIfNumbersAreNumbers: function () {
				...
				for (var i = 0; i < saveLineItemREEntryJCount; i++) {
					saveLineItemREEntryJ[i].MaterialCost = (saveLineItemREEntryJ[i].MaterialCost != null) ? saveLineItemREEntryJ[i].MaterialCost.split(
						',').join('') : saveLineItemREEntryJ[i].MaterialCost; // MAC_APS1157+
